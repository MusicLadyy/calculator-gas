...
def calculate_pseudocritical(composition):
    ...
    return round(P_kr, 4), round(T_kr, 2), round(omega_cm, 6)
...
def calculate_Z_peng_robinson(composition, Ppr, Tpr, omega_cm):
    ...
    B = 0.0778 * Ppr / Tpr
    return round(solve_cubic_Newton(A, B), 6)
...
def calculate_viscosity(Ppr, Tpr):
    """Вязкость (упрощённо)"""
    mu_atm = 0.0112
    return round(mu_atm * (1 + 0.5 * Ppr), 6)
...
def calculate_joule_thomson(Ppr, Tpr, Cp, P1, P2, T1):
    ...
    return round(Di, 4), round(delta_T, 2), round(T2, 2)
